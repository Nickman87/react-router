...
/***/ function(module, exports, __webpack_require__) {
	...
	var Router = _react2['default'].createClass({
	  ...
	  _createParentComponentsEntry: function _createParentComponentsEntry(newComponents, props) {
	    var result = {};
	  ...
	  _applyParentComponents: function _applyParentComponents(props, parentComponents) {
	    if (typeof parentComponents === 'undefined') return;

	    for (var key in parentComponents) {
	      if (parentComponents.hasOwnProperty(key)) {
	        var newComponent = parentComponents[key];

	        if (_react.isValidElement(newComponent)) {
	          //Just set the new component on the props
	          props[key] = newComponent;
	        } else {
	          if (!props.hasOwnProperty(key)) props[key] = {}; //Prepare an empty object

	          if (_react.isValidElement(props[key])) {
	            //Clone the element and inject the nex properties
	            props[key] = _react2['default'].cloneElement(props[key], newComponent);
	          } else {
	  ...
	  render: function render() {
	    ...
	    if (components) {
	      element = components.reduceRight(function (element, components, index) {
	        ...
	        if (route.hasOwnProperty('parent_components')) parentComponentsCache.unshift(_this2._createParentComponentsEntry(route['parent_components'], props));
	        ...
	        if (_react.isValidElement(element)) {
	          ...
	          //It is not a single element, but multiple, add them to the props
	        ...
	        for (var i = 0; i < parentComponentsCache.length; ++i) {
	          if (parentComponentsCache[i].hasOwnProperty(route['name'])) {
	            ...
	            _this2._applyParentComponents(props, parentComponents);
	          }
	        }
